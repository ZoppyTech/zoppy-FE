...
import { UpdateUserPasswordRequest } from 'src/shared/models/requests/user/update-password.request';
...
export class UserService extends ApiService {
    ...
    public async updatePassword(request: UpdateUserPasswordRequest): Promise<BooleanResponse> {
        const promise: Promise<BooleanResponse> = new Promise((resolve: any, reject: any) => {
            this.put<BooleanResponse, UpdateUserPasswordRequest>(`${this.url}/password`, request).subscribe(
                (response: BooleanResponse) => resolve(response),
                (error: ZoppyException) => reject(error)
            );
        });
